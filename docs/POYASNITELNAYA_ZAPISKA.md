...
> **Оформление для сдачи в Word:** выполните `py -3 docs/generate_poyasnitelnaya_zapiska.py` из корня репозитория (или из каталога `docs/`). В полученном `docs/POYASNITELNAYA_ZAPISKA.docx` заданы: поля страницы (левое 30 мм, правое 15 мм, верх/низ 20 мм), шрифт **Times New Roman**, основной текст **14 pt**, межстрочный полуторный интервал, **красная строка** 12,5 мм у обычных абзацев; заголовки разделов 1–14 и подразделы 5.1–5.4 — **чёрный** цвет (не синий стиль Word); заголовки разделов — **16 pt**, подразделы 5.x — **14 pt**.
...
- **Git** и **GitHub** — версионирование исходного кода, история изменений (`git log`), ветвление; публичный репозиторий служит **единым местом** для получения проекта, дублирует и дополняет текст пояснительной записки файлами `README.md`, `docs/`. Это распространённая практика передачи знаний между разработчиками и проверяющими.
