...
> **Оформление для сдачи в Word:** перенесите текст в Microsoft Word и задайте вручную: поля страницы (левое 30 мм, правое 15 мм, верх/низ 20 мм), шрифт **Times New Roman**, основной текст **14 pt**, межстрочный полуторный интервал, **красная строка** 12,5 мм у обычных абзацев; заголовки разделов 1–14 и подразделы 5.1–5.4 — **чёрный** цвет (не синий стиль Word); заголовки разделов — **16 pt**, подразделы 5.x — **14 pt**.
...
| Знания о коде и API | **GitHub** (репозиторий, история), **OpenAPI** (`/openapi.json`), **Swagger UI** и **ReDoc** (`/docs`, `/redoc`) — встроено в FastAPI |
...
В учебных и отчётных формулировках под **системами управления знаниями** (СУЗ) в сфере ИТ часто понимают средства фиксации, структурирования и совместного использования **знаний о продукте и процессе разработки** (не путать с **СУБД** из п. 7). Для отчёта уместно разделить: **СУБД** хранят данные приложения; **СУЗ** в этом смысле — репозитории, регламенты, описания API, конспекты, вики команды.

**Что можно упомянуть в отчёте как примеры класса СУЗ (организационно-методические):**

- **Корпоративные порталы и вики** — Confluence, Notion, Microsoft SharePoint, встроенная **Wiki** на GitHub: удобны для регламентов, дизайна, чек-листов при **командной** работе. В **данном** учебном проекте основной акцент сделан на репозитории и автогенерацию документации к API.

**Что используется по факту в проекте (инженерные «носители знаний»):**

- **Git** и **GitHub** — версионирование исходного кода, история изменений (`git log`), ветвление; публичный репозиторий служит **единым местом** для получения проекта, дублирует и дополняет текст пояснительной записки файлом **`README.md`** в корне репозитория. Это распространённая практика передачи знаний между разработчиками и проверяющими.
- **OpenAPI** (ранее Swagger Specification) — машиночитаемое описание REST API: схемы запросов/ответов, коды, параметры. Фреймворк **FastAPI** строит контракт из типов и аннотаций и отдаёт JSON-спецификацию (по умолчанию эндпоинт **`/openapi.json`**) и интерактивные страницы **`/docs`** (Swagger UI) и **`/redoc`** (ReDoc). Это снижает риск расхождения между «как написано» и «как работает сервер» и облегчает сопровождение клиентов (веб, Flutter).

**Итог для формулировки в отчёте:** в работе задействованы **GitHub** как платформа хранения и распространения знаний о коде и **OpenAPI** (через встроенные средства FastAPI) как формализованное описание знаний о контракте HTTP API.

---

## 9. Анализ соответствия принципам SOLID (фрагментарно, по коду backend)
...
## 10. Метод авторизации и контроль доступа на backend
...
12. GitHub — https://github.com/ (платформа хранения репозитория и сопутствующих материалов).
13. Спецификация OpenAPI — https://spec.openapis.org/oas/latest.html
14. Интерактивная документация FastAPI (Swagger UI / ReDoc) — https://fastapi.tiangolo.com/features/#automatic-docs
